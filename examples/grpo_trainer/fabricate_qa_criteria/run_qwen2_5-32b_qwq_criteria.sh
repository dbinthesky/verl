...
setup_path() {
    ...
    experiment_name="qwen2_5-32b_qwq_fabricate_qa_criteria-dlc-${YYMMDD}-${HHMMSS}"
    project_name="verl_grpo_qwq_fabricate_qa_criteria"

    OUTPUT_DIR="/cpfs01/shared/llm_ddd/tongjian/ckpts/datareview_rl_test/verl/grpo/qwen2_5-32b_qwq_fabricate_qa_criteria/${experiment_name}/${YYMMDD}/${HHMMSS}"
    mkdir -p "${OUTPUT_DIR}"
}
setup_path
...
run_training() {
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        custom_reward_function.name=qwq_longcot_create_criteria_compute_score_train \
        +custom_valid_reward_function.path="${CUSTOM_CODE_DIR}/rewards/fabricate_qa.py" \
        +custom_valid_reward_function.name=qwq_longcot_create_criteria_compute_score_valid \
