...
def create_device_mesh(world_size, fsdp_size):
    if fsdp_size < 0 or fsdp_size >= world_size:
        device_mesh = init_device_mesh(get_device_name(), mesh_shape=(
            world_size,), mesh_dim_names=["fsdp"])
    else:
        device_mesh = init_device_mesh(get_device_name(), mesh_shape=(
            world_size // fsdp_size, fsdp_size), mesh_dim_names=["ddp", "fsdp"])
    return device_mesh
...
class SPINRolloutRefWorker(ActorRolloutRefWorker):
    ...
    def init_model(self):
        ...
        if self._is_ref:
            ...
            self.checkpoint_manager = FSDPCheckpointManager(model=self.actor_module_fsdp, optimizer=self.actor.actor_optimizer, lr_scheduler=self.actor_lr_scheduler,
                                                            processing_class=self.processor if self.processor is not None else self.tokenizer, checkpoint_contents=self.config.actor.checkpoint)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(tensors={"old_log_probs": output}, meta_info={
    ...
    def update_actor_dpo(self, data: DataProto):
        ...
        log_gpu_memory_usage(
            "Before update policy (DPO via PPO path)", logger=logger)
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_dpo_via_ppo", logger=None) as timer:
                ...
                metrics = self.actor.update_policy_dpo_with_ref(
                    data=data)  # <-- THIS CALLS THE MODIFIED FUNCTION
            delta_time = timer.last

            # --- Add Performance Metrics ---
            # MFU calculation might be less accurate/meaningful here for DPO
            metrics["perf/approx_tokens_processed"] = torch.sum(data.batch.get(
                "attention_mask", torch.tensor(0))).item()  # Approx tokens
            ...
            metrics["perf/mfu/actor"] = estimated_flops * \
...
class RewardModelWorker(Worker):
    ...
    def __init__(self, config):
        ...
        if self.ulysses_sequence_parallel_size > 1:
            self.ulysses_device_mesh = init_device_mesh(get_device_name(), mesh_shape=(
                dp, self.ulysses_sequence_parallel_size), mesh_dim_names=["dp", "sp"])
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            reward_module = AutoModelForTokenClassification.from_pretrained(
                pretrained_model_name_or_path=local_path, config=model_config, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2", trust_remote_code=trust_remote_code)
